...
void ofApp::update() {
	...
	if (lsl.isActive()) {
		...
		if (buffer.size()) {
			...
			sendEmotiBitPacket(EmotiBitPacket::TypeTag::LSL_MARKER, ofToString(sampleToUse.timestamp, 15) + ss.str());
			cout << EmotiBitPacket::TypeTag::LSL_MARKER << "," << ofToString(sampleToUse.timestamp, 15) + ss.str() << endl;
		}
	}
